...
const resolveEmployeeId = async (db, userId) => {
...
const findTipeFromSearch = (search) => {
    const normalized = search.toLowerCase().replace(/\s+/g, "_");
    const exactTipe = tipeToPrisma[normalized];
    if (exactTipe) {
        return exactTipe;
    }
    ...
    return labelMatch ? tipeToPrisma[labelMatch[0]] : null;
};
const formatMyIzin = (izin) => {
    return {
        id: izin.id,
...
const ensurePendingIzin = async (db, id) => {
...
const findIzinForHrdResponse = async (db, id) => {
    const izin = await db.izin.findUnique({
        where: { id },
...
const calculateUsedCutiDays = (items, range) => {
...
export const createIzinService = (db = prisma) => {
    return {
        ...
        async submitIzin(userId, payload) {
            const employeeId = await resolveEmployeeId(db, userId);
        ...
        async getHrdSummary() {
            ...
            const [menunggu, disetujuiBulanIni, ditolakBulanIni, totalBulanIni] = await db.$transaction([
                db.izin.count({
                    where: {
                        status: "MENUNGGU",
                    },
        ...
        async approveIzin(id, hrdUserId) {
        ...
        async rejectIzin(id, hrdUserId, payload) {
            const result = await db.izin.updateMany({
                where: {
                    id,
                    status: "MENUNGGU",
