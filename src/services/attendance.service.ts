...
const getEmployeeIdByUserId = async (userId: string) => {
...
export const attendanceService = {
  ...
  async getRealtimeAttendance(query: RealtimeAttendanceQuery) {
    const today = startOfToday();
  ...
  async getMyMonthlyRecap(userId: string, query: MonthlyRecapQuery) {
    const employeeId = await getEmployeeIdByUserId(userId);
    const { start, end } = monthRange(query.month);

    const [hadir, terlambat, izin, alfa, belumAbsen] =
